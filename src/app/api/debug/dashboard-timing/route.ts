...
export async function GET() {
  ...
  if (!activeRepoStep.result) {
    return NextResponse.json({
      ...
      steps: [activeRepoStep],
    });
  }

  const repoId = activeRepoStep.result.id;

  const [eventsStep, postsStep, strategyStep, settingsStep, userStateStep, suggestionStep, monthlyCountStep, scheduledPostsStep, scheduledNewsStep] = await Promise.all([
    timed("events", () =>
